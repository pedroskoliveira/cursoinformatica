...
export const InstructorDashboard: React.FC<InstructorDashboardProps> = ({
  ...
  const handleUnlockQuiz = async (studentId: string, studentName: string) => {
    setUnlockingStudentId(studentId);
    try {
      const res = await fetch('/api/students/unlock-quiz', {
        method: 'POST',
      ...
      if (data.success) {
        setFeedbackMsg({
          type: 'success',
          text: `Quiz liberado com sucesso para ${studentName}! Todas as notas e histórico foram preservados.`,
        });
        if (selectedStudent && selectedStudent.id === studentId) {
          setSelectedStudent({ ...selectedStudent, currentStage: 'quiz' });
        }
        if (onRefresh) onRefresh();
      } else {
        setFeedbackMsg({
          type: 'error',
          text: data.error || 'Não foi possível liberar o quiz.',
        });
      }
    } catch {
      setFeedbackMsg({
        type: 'error',
        text: 'Erro de conexão ao liberar quiz.',
      });
    } finally {
  ...
  const handleUnlockQuizAll = async () => {
    const confirmMsg = 'Deseja liberar o Quiz para TODOS os alunos que estão na etapa de vídeo? Nenhuma nota ou progresso será apagado.';
    if (!window.confirm(confirmMsg)) return;

    setIsUnlockingAll(true);
    try {
      const res = await fetch('/api/students/unlock-quiz-all', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      const data = await res.json();
      if (data.success) {
        setFeedbackMsg({
          type: 'success',
          text: `Quiz liberado com sucesso para ${data.unlockedCount} aluno(s)!`,
        });
        if (onRefresh) onRefresh();
      } else {
        setFeedbackMsg({
          type: 'error',
          text: data.error || 'Erro ao liberar quiz para todos.',
        });
      }
    } catch {
      setFeedbackMsg({
        type: 'error',
        text: 'Erro de conexão ao liberar quiz em lote.',
      });
    } finally {
  ...
  return (
    <div id="admin-dashboard-wrapper" className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      {/* Top Banner with Real-Time Notification Message */}
      {feedbackMsg && (
        <div
          id="instructor-action-feedback-toast"
      ...
      {activeTab === 'roster' && (
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-4 rounded-2xl">
            ...
            {/* Quick Bulk Unlock Action */}
            {students.some((s) => s.currentStage === 'video') && (
              <button
                type="button"
                id="unlock-quiz-all-students-btn"
                disabled={isUnlockingAll}
                onClick={handleUnlockQuizAll}
                className="px-3.5 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-xl text-xs font-bold flex items-center space-x-1.5 shadow-md shadow-blue-600/20 transition-all cursor-pointer hover:scale-105 active:scale-95 whitespace-nowrap"
                title="Libera o Quiz imediatamente para todos os alunos que estão na etapa de vídeo sem apagar nada"
              >
                <Unlock className="w-3.5 h-3.5 text-blue-200" />
                <span>
                  Liberar Quiz p/ Quem Assistiu ({students.filter((s) => s.currentStage === 'video').length})
                </span>
              </button>
            )}
          </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-5 py-4 text-right space-x-1.5 whitespace-nowrap">
                            {/* Liberar Quiz Manualmente (Mantém notas e histórico) */}
                            {s.currentStage === 'video' ? (
                              <button
                                type="button"
                                id={`unlock-quiz-${s.id}-btn`}
                                disabled={unlockingStudentId === s.id}
                                onClick={() => handleUnlockQuiz(s.id, s.name)}
                                className="px-2.5 py-1.5 text-xs text-blue-200 hover:text-white bg-blue-600 hover:bg-blue-500 rounded-lg shadow-sm font-bold transition-all inline-flex items-center space-x-1 cursor-pointer hover:scale-105 active:scale-95"
                                title="Liberar Quiz imediatamente para este aluno (mantém histórico e notas intactos)"
                              >
                                <Unlock className="w-3.5 h-3.5" />
                                <span>Liberar Quiz</span>
                              </button>
                            ) : s.currentStage === 'quiz' ? (
                              <span className="px-2 py-1 text-[11px] font-bold text-emerald-300 bg-emerald-950/60 border border-emerald-500/30 rounded-lg inline-flex items-center space-x-1">
                                <CheckCircle2 className="w-3 h-3 text-emerald-400" />
                                <span>Quiz Liberado</span>
                              </span>
                            ) : null}
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between space-x-2 pt-2 border-t border-slate-800">
              {selectedStudent.currentStage === 'video' ? (
                <button
                  type="button"
                  id="modal-unlock-quiz-student-btn"
                  disabled={unlockingStudentId === selectedStudent.id}
                  onClick={() => handleUnlockQuiz(selectedStudent.id, selectedStudent.name)}
                  className="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center space-x-2 shadow-md hover:scale-105 active:scale-95"
                >
                  <Unlock className="w-4 h-4 text-blue-200" />
                  <span>Liberar Quiz do Dia {selectedStudent.currentDay} Agora</span>
                </button>
              ) : (
                <span className="text-xs font-bold text-emerald-400 flex items-center space-x-1.5">
                  <CheckCircle2 className="w-4 h-4" />
                  <span>Etapa do aluno: {selectedStudent.currentStage === 'quiz' ? 'Quiz Liberado' : 'Curso Concluído'}</span>
                </span>
              )}
              <button
                type="button"
